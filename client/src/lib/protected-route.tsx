...
/**
 * Admin-and-up only. The money pages' APIs already 403 non-admins; this stops a
 * staff deep-link from rendering a broken page shell of failed requests — they land
 * back on the orders board instead.
 */
export function AdminProtectedRoute({
  ...
  if (!['admin', 'super_admin'].includes(user.role)) {
    return (
      <Route path={path}>
        <Redirect to={user.role === 'staff' ? '/staff-portal/orders-board' : '/'} />
      </Route>
    );
  }
