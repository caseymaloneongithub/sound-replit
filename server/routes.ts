      ...
      const orders = [
        ...retail.map(o => ({
          ...
          items: o.items.flatMap(i => {
            // A split case lands as 0.5 under EACH chosen flavor column (owner,
            // 2026-09-02) instead of 1 under Mixed. The note is generated by
            // splitItemFields, so the format is ours to parse.
            const split = /^Split: 6 (.+) \/ 6 (.+)$/.exec(i.notes ?? '');
            if (split) {
              const unit = i.unitDescription ? ` — ${canonUnit(i.unitDescription)}` : '';
              return [
                { label: `${split[1]}${unit}`, quantity: i.quantity / 2, note: i.notes },
                { label: `${split[2]}${unit}`, quantity: i.quantity / 2, note: null },
              ];
            }
            return [{
